...
_SYSTEM_PROMPT_V40 = (
    ...
    "- 【文本清洗铁律 · 最高优先级】若原文出现排版破碎的乱码数字，例如 '-/31'、'.0'、'/1'、'6/1-/31'、'CEISA .0'——绝对禁止原样输出这些残缺符号！必须强制将其转化为定性描述。示例映射：\n"
    "    · 残缺日期 '6/1-/31' → 改写为『6月至8月底的过渡期』\n"
    "    · 残缺版本号 'CEISA .0' → 改写为『新版海关申报系统』\n"
    "    · 残缺数字 '/1评估' → 改写为『9月初进行综合评估』\n"
    "  宁可丢失精确数字，也不可输出残缺碎片。高管终端不容任何乱码。\n"
